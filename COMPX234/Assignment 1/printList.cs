...
class printList
{
    ...
    public printList queueInsert(printList list, printDoc doc)
    {
        ...
        if (list.head == null)
        {
            list.head = new_node;
            Console.WriteLine(
                "Inserted a request in the queue from " + new_node.document.getSender()
            );
    ...
    public void queuePrint(printList list, int id)
    {
        ...
        if (list.head != null)
        {
            ...
            Console.WriteLine(
                "Printer "
                    + id
                    + " Printing the request from Machine ID: "
                    + currNode.document.getSender()
                    + " "
                    + currNode.document.getStr()
                    + " "
            );
            Console.WriteLine(":::::");
            Console.Out.Flush();
